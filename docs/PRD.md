...
| Vacinas e Vermífugos | Registro de vacinas, vermífugos, histórico, reforços, comprovantes | 🔴 P0 |
| Medicamentos e Vitaminas | Controle de administração, dosagem, horários para remédios e vitaminas | 🔴 P0 |
...
| Cadastro de Veterinários | Perfil profissional com CRMV, modalidades e horários | 🟡 P1 |
| Busca de Veterinários | Filtros por espécie, proximidade, convênio e favoritos | 🟡 P1 |
...
| Agendamento online com veterinário | Fase 3 | Depende de base de veterinários cadastrados |
...
| M02-F05 | Histórico de Peso | Registrar histórico, editar, excluir aferições de peso | P0 |
| M02-F06 | Arquivamento | Ocultar perfil sem excluir dados (ex: pet falecido) | P1 |
| M02-F07 | QR Code do pet | Gerar QR com identificação para coleira | P2 |
| M02-F08 | Compartilhamento de perfil | Compartilhar perfil do pet com outro usuário | P2 |
...
## M03 — Vacinas e Vermífugos

### 7.3.1 Descrição
Módulo para registro e controle completo da carteirinha vacinal e histórico de vermífugos de cada pet, incluindo reforços automáticos e anexo de comprovantes digitais.
...
- [ ] Histórico vacinal e de vermífugos é exibido em ordem cronológica reversa (mais recente primeiro).
- [ ] Layout da tela padronizado seguindo o modelo de controle de medicamentos.
...
## M05 — Medicamentos e Vitaminas

### 7.5.1 Descrição
Módulo para gerenciamento completo da administração de remédios e vitaminas, com controle de dosagem, horários e acompanhamento da aderência.
...
| `medication_type` | Enum | Sim | `vaccine`, `vitamin`, `dewormer`, `medicine` |
...
### 7.9.1 Descrição
Módulo de cadastro e descoberta de médicos-veterinários, permitindo que profissionais criem perfis verificados por CRMV e que tutores os encontrem por proximidade, espécie atendida, modalidade de atendimento e tipo de pagamento aceito.

### 7.9.2 Modelo de Dados — Veterinário

| Campo | Tipo | Obrigatório | Validação |
|---|---|---|---|
| `id` | UUID v4 | Auto | — |
| `user_id` | UUID v4 | Sim | FK → Users (conta PetLife do vet) |
| `crmv_number` | String(20) | Sim | Formato: `CRMV-UF-NNNNN` (ex: CRMV-SP-12345) |
| `crmv_state` | String(2) | Sim | UF de registro (SP, RJ, MG...) |
| `crmv_status` | Enum | Auto | `pending`, `verified`, `rejected` |
| `full_name` | String(200) | Sim | Nome completo do profissional |
| `bio` | Text | Não | Descrição e apresentação do profissional |
| `specialties` | JSON Array | Não | Ex: ["Clínica geral", "Cirurgia", "Dermatologia"] |
| `species_served` | JSON Array | Sim | Enum values: `dog`, `cat`, `bird`, `exotic`, `reptile`, `rodent`, `wildlife`, `other` |
| `modalities` | JSON Array | Sim | `clinic`, `individual`, `home_visit` |
| `payment_types` | JSON Array | Sim | `private`, `insurance` |
| `insurance_plans` | JSON Array | Não | Ex: ["PetAmigo", "VidaPet"] |
| `emergency_on_duty` | Boolean | Não | Default: false — Disponível para plantão |
| `availability_status` | Enum | Não | `available`, `unavailable` — Status do dia |
| `profile_photo_url` | String(500) | Não | URL da foto do profissional |
| `phone` | String(20) | Não | Telefone de contato |
| `website_url` | String(500) | Não | — |
| `created_at` | Timestamp | Auto | — |
| `updated_at` | Timestamp | Auto | — |

### 7.9.3 Modelo de Dados — Endereço do Veterinário

| Campo | Tipo | Obrigatório | Validação |
|---|---|---|---|
| `id` | UUID v4 | Auto | — |
| `veterinarian_id` | UUID v4 | Sim | FK → Veterinarians |
| `label` | String(100) | Não | Ex: "Clínica Principal", "Consultório Filial" |
| `street` | String(300) | Sim | — |
| `number` | String(20) | Sim | — |
| `complement` | String(100) | Não | — |
| `neighborhood` | String(100) | Sim | — |
| `city` | String(100) | Sim | — |
| `state` | String(2) | Sim | — |
| `postal_code` | String(10) | Sim | Formato: `NNNNN-NNN` |
| `latitude` | Decimal(10,8) | Auto | Geocodificado pelo backend |
| `longitude` | Decimal(11,8) | Auto | Geocodificado pelo backend |
| `is_primary` | Boolean | Não | Default: true |

### 7.9.4 Modelo de Dados — Horários de Atendimento

| Campo | Tipo | Obrigatório | Validação |
|---|---|---|---|
| `id` | UUID v4 | Auto | — |
| `veterinarian_id` | UUID v4 | Sim | FK → Veterinarians |
| `day_of_week` | Enum | Sim | `MONDAY`, `TUESDAY`, `WEDNESDAY`, `THURSDAY`, `FRIDAY`, `SATURDAY`, `SUNDAY` |
| `open_time` | Time | Sim | Formato: HH:MM |
| `close_time` | Time | Sim | Formato: HH:MM, deve ser > open_time |
| `is_active` | Boolean | Não | Default: true |

### 7.9.5 Modelo de Dados — Favoritos do Tutor

| Campo | Tipo | Obrigatório |
|---|---|---|
| `id` | UUID v4 | Auto |
| `user_id` | UUID v4 | Sim |
| `veterinarian_id` | UUID v4 | Sim |
| `created_at` | Timestamp | Auto |

### 7.9.6 Funcionalidades

**Para o Veterinário:**

| ID | Funcionalidade | Descrição | Prioridade |
|---|---|---|---|
| M09-F01 | Cadastro de perfil profissional | Registro com CRMV, especialidades, espécies e modalidades | P1 |
| M09-F02 | Verificação de CRMV | Validação do número contra padrão de formato; status `pending` → `verified` | P1 |
| M09-F03 | Cadastro de endereços | Um ou mais endereços com geocodificação automática | P1 |
| M09-F04 | Gestão de horários | Definição de dias e horários de atendimento por dia da semana | P1 |
| M09-F05 | Status de disponibilidade | Toggle diário `available` / `unavailable` | P1 |
| M09-F06 | Plantão de emergência | Toggle para indicar disponibilidade para emergências | P1 |
| M09-F07 | Edição de perfil | Atualizar qualquer campo do perfil profissional | P1 |

**Para o Tutor:**

| ID | Funcionalidade | Descrição | Prioridade |
|---|---|---|---|
| M09-F08 | Busca de veterinários | Pesquisa com múltiplos filtros combinados | P1 |
| M09-F09 | Filtro por espécie | Exibir apenas vets que atendem a espécie do pet selecionado | P1 |
| M09-F10 | Filtro por proximidade | Ordenação por distância com base na localização do tutor | P1 |
| M09-F11 | Filtro por modalidade | Clínica, atendimento individual, domiciliar | P1 |
| M09-F12 | Filtro por pagamento | Particular ou convênio (com nome do plano) | P1 |
| M09-F13 | Busca por emergência | Filtrar apenas vets de plantão disponíveis agora | P1 |
| M09-F14 | Favoritar veterinário | Salvar veterinário na lista de preferências do tutor | P2 |
| M09-F15 | Ver perfil do veterinário | Visualizar dados completos, horários e endereço | P1 |

### 7.9.7 Regras de Negócio

- **Validação de CRMV:** O sistema valida o formato `CRMV-UF-NNNNN` via regex. A verificação de autenticidade do CRMV junto ao CFMV é realizada manualmente pela equipe (status `pending` → `verified` por administradores).
- **Geocodificação:** Ao salvar um endereço, o backend deve geocodificar o CEP via API externa (ViaCEP para dados do endereço, e Nominatim/Google Geocoding para coordenadas) e persistir latitude/longitude.
- **Filtro por Proximidade:** A consulta de busca usa a fórmula de Haversine (ou extensão PostGIS) para ordenar por distância geográfica a partir da localização do tutor.
- **Disponibilidade Diária:** O campo `availability_status` é manual e reset-ável pelo veterinário a qualquer momento. Não é calculado automaticamente.
- **Plantão de Emergência:** Um vet marcado com `emergency_on_duty = true` E `availability_status = available` é elegível para o filtro de emergência.
- **Unicidade de CRMV:** Não podem existir dois perfis com o mesmo número de CRMV (unique constraint no banco).
- **Cascata:** Ao excluir a conta do veterinário, todos os endereços, horários e favoritos relacionados são removidos (LGPD).

### 7.9.8 Critérios de Aceitação

- [x] Veterinário pode criar perfil informando CRMV com o formato correto (ex: `CRMV-SP-12345`).
- [x] Sistema rejeita CRMVs com formato inválido na criação do perfil.
- [x] Perfil fica com status `pending` até verificação manual da equipe.
- [x] Veterinário pode cadastrar um ou mais endereços com geocodificação automática de coordenadas.
- [x] Veterinário pode definir horários por dia da semana (ex: Segunda 08:00–18:00).
- [x] Veterinário pode alternar status `available` / `unavailable` a qualquer momento.
- [x] Veterinário pode ativar/desativar plantão de emergência.
- [x] Tutor pode buscar veterinários filtrando por espécie atendida.
- [x] Tutor pode buscar veterinários ordenados por distância (requer permissão de localização).
- [x] Tutor pode filtrar por modalidade (clínica, individual, domiciliar).
- [x] Tutor pode filtrar por tipo de pagamento (particular, convênio) e pelo nome do plano.
- [x] Tutor pode filtrar apenas veterinários de plantão e disponíveis.
- [x] Tutor pode favoritar/desfavoritar um veterinário.
- [x] Tutor pode visualizar a lista de favoritos em seção dedicada.
- [x] Perfil público do veterinário exibe CRMV, especialidades, espécies atendidas, horários e endereço(s).
- [x] CRMV de veterinários não verificados é exibido com badge `Em verificação`.
...
- [x] Linha do tempo carrega com scroll infinito (20 eventos por página).
- [x] Eventos são exibidos em ordem cronológica reversa (mais recente no topo).
- [x] Cada evento mostra ícone, tipo, data, resumo e foto (se existir).
- [x] Tutor pode filtrar por tipo de evento.
- [x] Tutor pode navegar para o detalhe do evento ao tocar nele.
- [x] Aniversários do pet são gerados automaticamente a cada ano.
...
- [x] Push notifications funcionam com o app fechado (Android e iOS).
- [x] Tutor pode ativar/desativar cada tipo de notificação individualmente.
- [x] Central de notificações exibe todas as notificações recentes com status (lida/não lida).
- [x] Notificações de medicamento incluem botão de ação rápida "Marcar como tomado".
- [x] Respeita fuso horário do dispositivo do usuário.
...
erDiagram
    ...
    USERS ||--o{ VETERINARIANS : "perfil profissional"
    USERS ||--o{ VET_FAVORITES : "favorita"
    VETERINARIANS ||--o{ VET_ADDRESSES : "possui"
    VETERINARIANS ||--o{ VET_SCHEDULES : "define horários"
    VETERINARIANS ||--o{ VET_FAVORITES : "é favoritado por"
    PETS {
        uuid id PK
        uuid user_id FK
        string name
...
| `GET` | `/api/v1/auth/me` | Retornar dados do perfil do usuário logado |
| `PUT` | `/api/v1/auth/me` | Atualizar dados do perfil do usuário logado |
| `POST` | `/api/v1/auth/me/photo` | Atualizar foto do perfil (upload) |
...
| Método | Endpoint | Descrição |
|---|---|---|
| `POST` | `/api/v1/veterinarians` | Criar perfil de veterinário |
| `GET` | `/api/v1/veterinarians/me` | Ver meu perfil de veterinário |
| `PUT` | `/api/v1/veterinarians/me` | Atualizar meu perfil |
| `PATCH` | `/api/v1/veterinarians/me/availability` | Atualizar status de disponibilidade |
| `PATCH` | `/api/v1/veterinarians/me/emergency` | Ativar/desativar plantão de emergência |
| `POST` | `/api/v1/veterinarians/me/addresses` | Adicionar endereço |
| `PUT` | `/api/v1/veterinarians/me/addresses/:id` | Atualizar endereço |
| `DELETE` | `/api/v1/veterinarians/me/addresses/:id` | Remover endereço |
| `POST` | `/api/v1/veterinarians/me/schedules` | Definir horários de atendimento |
| `PUT` | `/api/v1/veterinarians/me/schedules/:id` | Atualizar horário |
| `DELETE` | `/api/v1/veterinarians/me/schedules/:id` | Remover horário |
| `GET` | `/api/v1/veterinarians/search` | Buscar veterinários (filtros via query params) |
| `GET` | `/api/v1/veterinarians/:id` | Ver perfil público do veterinário |
| `POST` | `/api/v1/veterinarians/:id/favorite` | Favoritar veterinário |
| `DELETE` | `/api/v1/veterinarians/:id/favorite` | Desfavoritar veterinário |
| `GET` | `/api/v1/veterinarians/favorites` | Listar veterinários favoritos |
...
## Épico 9 — Cadastro e Busca de Veterinários

| ID | User Story | Prioridade | Pontos |
|---|---|---|---|
| US-034 | Como veterinário, quero criar meu perfil profissional com meu número de CRMV para ser encontrado por tutores | P1 | 8 |
| US-035 | Como veterinário, quero cadastrar meu(s) endereço(s) de atendimento para que tutores me localizem | P1 | 5 |
| US-036 | Como veterinário, quero definir meus horários de atendimento por dia da semana | P1 | 5 |
| US-037 | Como veterinário, quero informar quais espécies atendo para aparecer nos filtros corretos | P1 | 3 |
| US-038 | Como veterinário, quero indicar minhas modalidades (clínica, individual, domiciliar) e tipos de pagamento aceitos | P1 | 3 |
| US-039 | Como veterinário, quero atualizar meu status de disponibilidade (disponível/indisponível) no dia | P1 | 2 |
| US-040 | Como veterinário, quero ativar/desativar meu status de plantão de emergência | P1 | 2 |
| US-041 | Como tutor, quero buscar veterinários filtrando por espécie do meu pet | P1 | 5 |
| US-042 | Como tutor, quero buscar veterinários ordenados por proximidade da minha localização | P1 | 8 |
| US-043 | Como tutor, quero filtrar veterinários por modalidade de atendimento (clínica, domiciliar) | P1 | 3 |
| US-044 | Como tutor, quero filtrar veterinários por tipo de pagamento (particular ou convênio) | P1 | 3 |
| US-045 | Como tutor, quero buscar veterinários de plantão disponíveis para atendimento de emergência | P1 | 5 |
| US-046 | Como tutor, quero favoritar veterinários e acessar minha lista de favoritos rapidamente | P2 | 3 |
| US-047 | Como tutor, quero visualizar o perfil completo de um veterinário com suas especialidades, horários e endereço | P1 | 3 |

**Total de Story Points do MVP:** ~148 pontos  
**Total de Story Points (incluindo Épico 9):** ~206 pontos
...
gantt
    ...
    M09 Cadastro de Veterinários  :f3a, 2027-02-01, 25d
    M09 Busca e Filtros           :f3b, after f3a, 20d
    Compartilhamento vet          :f3c, after f3a, 20d
    Integração clínicas (API)     :f3d, after f3b, 30d
    Plano Premium (pagamento)     :f3e, after f3c, 20d
    Agenda compartilhada          :f3f, after f3d, 15d
...
- **Cadastro de Veterinários (M09):** Perfis profissionais verificados por CRMV com horários, modalidades e endereços
- **Busca e Descoberta de Veterinários (M09):** Filtros por espécie, proximidade, modalidade, convênio e plantão
...
| **Veterinário** | Médico-veterinário registrado no CRMV com perfil profissional na plataforma |
| **CRMV** | Conselho Regional de Medicina Veterinária — órgão regulador da profissão |
| **CFMV** | Conselho Federal de Medicina Veterinária — autarquia federal reguladora |
| **Plantão de Emergência** | Status voluntário do veterinário indicando disponibilidade imediata para atendimentos urgentes |
| **Modalidade de Atendimento** | Forma de prestação do serviço: clínica, consultório individual ou domiciliar |
| **Haversine** | Fórmula matemática para calcular distância entre dois pontos geográficos usando latitude/longitude |
