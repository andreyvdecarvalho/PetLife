...
public class AddVaccinationUseCase {
    ...
    public AddVaccinationUseCase(VaccinationPort vaccinationPort, PetRepositoryPort petPort) {
        this.vaccinationPort = vaccinationPort;
        this.petPort = petPort;
    }

    public VaccinationResponse execute(UUID petId, UUID userId, CreateVaccinationRequest request) {
        ...
        if (!pet.getUser().getId().equals(userId)) {
            throw BusinessException.forbidden("PET_FORBIDDEN", 
                    "You don't have permission to add vaccination to this pet");
        }
        ...
        vaccination.setCreatedAt(OffsetDateTime.now());
